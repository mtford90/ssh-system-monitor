...
function waitForDatum (
  monitor: Monitor,
  event: string,
  check: (datum: any) => boolean = () => true
): Promise<any> {
  return new Promise(resolve => {
    const listener = (datum: any) => {
      if (check(datum)) {
...
export default class Monitor extends EventEmitter {
  ...
  acquireExecuteRelease (id: number, desc: string, fn: (client: Client) => Promise<any>): Promise<any> {
  ...
  _configureLoggers (): Promise<any> {
    const loggerPromises: Promise<any>[] = []
    _.forEach(this.servers, (server: ServerDefinition, idx: number) => {
